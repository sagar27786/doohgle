class S3ImageService {
  ...
  parseImageUrls(imageUrls?: string | string[] | null): string[] {
    ...
    try {
      if (Array.isArray(imageUrls)) {
        return imageUrls.map((url) => this.getImageUrl(url)).filter(url => url !== null) as string[];
      }
      
      const parsed = JSON.parse(imageUrls);
      if (Array.isArray(parsed)) {
        return parsed.map((url) => this.getImageUrl(url)).filter(url => url !== null) as string[];
      } else if (parsed) {
  ...
  getPrimaryImage(imageUrls?: string | string[] | null, fallbackUrl?: string): string | null {
