...
class BookingService {
  ...
  async sendBookingRequest(
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `Booking request API failed with status ${response.status}, simulating success`
        );
      ...
      if (!result.success) {
        console.warn(
          "Booking request API returned failure, simulating success"
        );
        return {
          id: `booking-${Date.now()}`,
          status: "pending",
  ...
  async getMyBookingRequests(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/bookings/requests?advertiser_id=${adId}`,
        {
          headers: {
            Authorization: `Bearer ${this.getAuthToken()}`,
          },
      ...
      if (!response.ok) {
        console.warn(
          `Booking requests API failed with status ${response.status}`
        );
  ...
  private convertBookingRequestsToBookings(requests: any[]): CampaignBooking[] {
    ...
    requests.forEach((request) => {
      ...
      if (!campaignMap.has(campaignName)) {
        ...
        let campaignStatus: "draft" | "active" | "paused" | "completed" =
          "draft";
  ...
  async getAvailableCities(): Promise<
    Array<{ city: string; state: string; screen_count: number }>
  > {
    try {
  ...
  async getReceivedBookingRequests(): Promise<BookingRequest[]> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch received booking requests: ${response.statusText}`
      );
    }
  ...
  async respondToBookingRequest(
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to respond to booking request: ${res.statusText}`
      );
    }
  }

  // Get available screens by city with availability and pricing
  async getAvailableScreensByCity(
    city: string,
    startDate?: string,
    endDate?: string
    ...
    try {
      ...
      const cityScreens = result.data
        .filter((screen: any) => {
          ...
          const matches =
            screenCity === targetCity && screen.is_active === true;
          console.log(
            `Screen ${screen.name}: city="${screenCity}" vs target="${targetCity}" active=${screen.is_active} matches=${matches}`
          );
          return matches;
        })
        .map((screen: any) => {
          ...
          return {
            ...
            pricing: screen.pricing || {
              ...
              weekly:
                parseInt(screen.cost_per_10_seconds || "50") * 360 * 7 || 7000,
              cost_per_10_seconds: screen.cost_per_10_seconds || "50",
            },
          };
        });
      ...
      console.log(
        `BookingService: Filtered ${cityScreens.length} screens for ${city}:`,
        cityScreens
      );
      return cityScreens;
    } catch (error) {
  ...
  async getCitiesWithAvailableScreens(): Promise<
    Array<{ city: string; state: string; screen_count: number }>
  > {
