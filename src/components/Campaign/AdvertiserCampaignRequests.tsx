...
const AdvertiserCampaignRequests: React.FC<AdvertiserCampaignRequestsProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">My Campaign Requests</h2>
      ...
      {requests.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No campaign requests</h3>
          <p className="text-gray-500">Create a campaign to send requests to venue owners.</p>
        </div>
      ) : (
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="font-medium">
                      {campaignRequestService.calculateDuration(request.start_date, request.end_date)}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                ...
                {request.campaign_description && (
                  <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div className="text-sm text-gray-800">{request.campaign_description}</div>
                  </div>
                )}
                ...
                {request.status === 'rejected' && request.rejection_reason && (
                  <div className="mb-4 p-3 bg-red-50 rounded-lg border-l-4 border-red-200">
                    <div className="text-sm text-red-800 font-medium mb-1">Rejection Reason</div>
                ...
                {request.status === 'pending' && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                    <div className="flex items-center gap-2 text-yellow-800">
                      ...
                      <span className="text-sm font-medium">Awaiting venue owner response</span>
                    </div>
                    <p className="text-sm text-yellow-700 mt-1">
                      Your request has been sent to {request.venue_owner_name}. You'll be notified when they respond.
                    </p>
                  </div>
                )}
                ...
                {request.status === 'approved' && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    <div className="flex items-center gap-2 text-green-800">
                      ...
                      <span className="text-sm font-medium">Campaign request approved!</span>
                    </div>
                    <p className="text-sm text-green-700 mt-1">
                      Great! Your campaign has been approved and will go live as scheduled.
                    </p>
                  </div>
                )}
