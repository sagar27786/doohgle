...
import {
  TrendingUp, TrendingDown, Eye, MousePointer, Play, Pause,
  Calendar, DollarSign, Users, Target, Monitor, MapPin,
  Download, Filter, RefreshCw, Settings, Bell, Search,
  BarChart3, Activity, Globe, Zap, Star, Award
} from 'lucide-react';
...
const EnhancedDashboard: React.FC = () => {
  ...
  const KPICard: React.FC<{
    ...
    decimals?: number;
    delay?: number;
  }> = ({ title, value, change, icon: Icon, color, prefix, suffix, decimals, delay = 0 }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {chartData.topPerformingCampaigns.map((campaign, index) => (
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-semibold text-gray-600">{campaign.performance}%</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <ChartContainer title="Real-time Activity" delay={1.5}>
          <div className="space-y-4">
            {[
              { time: '2 min ago', action: 'New campaign started', type: 'success' },
              { time: '5 min ago', action: 'Screen went offline', type: 'warning' },
              { time: '8 min ago', action: 'Budget threshold reached', type: 'info' },
              { time: '12 min ago', action: 'High CTR detected', type: 'success' },
