...
async function getTransporter(): Promise<nodemailer.Transporter> {
  if (!transporterPromise) {
    transporterPromise = (async () => {
      ...
      console.log(
        "SMTP transporter configured for:",
...
export async function verifyMojoAuthOTP(req: Request, res: Response) {
  ...
  if (!state_id || !otp || !password || !confirmPassword) {
    return res.status(400).json({
      message: "State ID, OTP, password, and confirmPassword are required.",
    });
  }
  ...
  try {
    ...
    const mojoAuthUser = await mojoAuthBackendService.verifyEmailOTP(
      state_id,
      otp
    );
    ...
    if (!mojoAuthUser.user_profile?.email) {
      return res
        .status(400)
        .json({ message: "Email not found in MojoAuth response." });
    }
