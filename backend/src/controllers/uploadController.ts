...
const deleteFromS3 = async (keyOrUrl: string): Promise<void> => {
  try {
    const success = await deleteFileFromS3(keyOrUrl);
    if (!success) {
 ...
 */
export const uploadSingleMedia = async (req: MulterRequest, res: Response) => {
  try {
    if (!req.file) {
 ...
 */
export const uploadScreenMedia = async (req: MulterRequest, res: Response) => {
  try {
