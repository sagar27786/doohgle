...
pool
  ...
  .catch((err) => {
    ...
    if ((err as any)?.code === "ETIMEDOUT") {
      console.error(
        `❌ PostgreSQL connection timed out when connecting to ${
          dbHost || "your host"
        }:${dbPort || "5432"}`
      );
      console.error(
        "  - Check that the database host is reachable from this machine"
      );
      console.error(
        "  - Ensure the database allows connections from your IP / VPC"
      );
      console.error(
        "  - Verify the port (usually 5432) is open and not blocked by a firewall"
      );
    }
