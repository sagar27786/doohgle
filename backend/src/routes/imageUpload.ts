...
// Upload images to S3
async function uploadImageToS3(file: Express.Multer.File, screenId: string): Promise<string> {
  const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
  const key = `screens/screen-${screenId}-${uniqueSuffix}${path.extname(file.originalname)}`;
