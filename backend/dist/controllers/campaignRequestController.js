...
exports.approveCampaignRequest = approveCampaignRequest;
exports.rejectCampaignRequest = rejectCampaignRequest;
exports.getNotifications = getNotifications;
exports.markNotificationRead = markNotificationRead;
exports.getCampaignRequest = getCampaignRequest;
const db_1 = require("../db");
// Create a new campaign request (Advertiser)
async function createCampaignRequest(req, res) {
    ...
    try {
        ...
        const { screen_id, campaign_name, campaign_description, start_date, end_date, requested_hours = [], budget_offered, creative_assets = [], target_audience, notes, } = req.body;
        ...
        if (!screen_id ||
            ...
            return res.status(400).json({
                success: false,
                message: "Missing required fields: screen_id, campaign_name, start_date, end_date, budget_offered",
            });
        }
    ...
    `;
        ...
        res.json({
            ...
            data: result.rows.map((row) => ({
                ...row,
                creative_assets: typeof row.creative_assets === "string"
...
async function rejectCampaignRequest(req, res) {
    ...
    `, [
            ...
            `Your campaign request for "${campaignRequest.screen_name}" has been rejected. Reason: ${rejection_reason || "No reason provided"}`,
            requestId,
        ]);
        res.json({
            success: true,
...
async function getNotifications(req, res) {
    ...
    `;
        ...
        // Get unread count
        const unreadResult = await db_1.pool.query("SELECT COUNT(*) as unread_count FROM notifications WHERE user_id = $1 AND is_read = false", [userId]);
        res.json({
            success: true,
            data: result.rows,
...
// Get campaign request details
async function getCampaignRequest(req, res) {
    try {
        const userId = req.user?.id;
        const requestId = req.params.id;
        if (!userId) {
    ...
    `;
        const result = await db_1.pool.query(query, [requestId, userId]);
        ...
        res.json({
            ...
            data: {
                ...data,
                creative_assets: typeof data.creative_assets === "string"
