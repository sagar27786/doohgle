...
exports.getAdsManagerNotifications = getAdsManagerNotifications;
exports.markNotificationAsRead = markNotificationAsRead;
exports.createBookingWithAuth = createBookingWithAuth;
exports.checkScreenAvailability = checkScreenAvailability;
exports.getMyBookings = getMyBookings;
exports.cancelBooking = cancelBooking;
const db_1 = require("../db");
const awsNotificationService_1 = require("../services/awsNotificationService");
// Create new booking request with enhanced location-wide notifications
async function createBooking(req, res) {
    const client = await db_1.pool.connect();
    try {
        await client.query("BEGIN");
        const { screenId, customerName, customerEmail, customerPhone, companyName, startDate, endDate, urgency, message, } = req.body;
        // Get screen details
        const screenQuery = `
    ...
    `;
        ...
        const days = Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) /
            (1000 * 60 * 60 * 24));
    ...
    `;
      ...
      `;
        ...
        const notificationPromises = locationOwners.map(async (owner) => {
            ...
            return awsNotificationService_1.awsNotificationService.sendBookingNotification(notificationData);
        });
...
async function getOwnerBookingRequests(req, res) {
...
async function processBookingApproval(req, res) {
    const client = await db_1.pool.connect();
    try {
        await client.query("BEGIN");
...
async function getAdsManagerNotifications(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function markNotificationAsRead(req, res) {
    ...
    try {
      ...
      RETURNING *
    `;
        const result = await db_1.pool.query(query, [notificationId, userId]);
...
async function createBookingWithAuth(req, res) {
...
async function checkScreenAvailability(req, res) {
    try {
      ...
      `;
            const conflictResult = await db_1.pool.query(conflictQuery, [
                screen_id,
                start_date,
                end_date,
            ]);
...
async function cancelBooking(req, res) {
    ...
    try {
      ...
      RETURNING *
    `;
        const result = await db_1.pool.query(query, [booking_id, userId]);
        if (result.rows.length === 0) {
            return res
                .status(404)
