...
exports.checkScreenAvailability = checkScreenAvailability;
exports.createBooking = createBooking;
exports.getMyBookings = getMyBookings;
exports.cancelBooking = cancelBooking;
exports.sendBookingRequest = sendBookingRequest;
exports.getMyBookingRequests = getMyBookingRequests;
exports.getAvailableCities = getAvailableCities;
exports.getBookedTimeSlots = getBookedTimeSlots;
exports.getBookingRequestsForOwner = getBookingRequestsForOwner;
const db_1 = require("../db");
// Check screen availability for given time slots
async function checkScreenAvailability(req, res) {
    try {
        ...
        try {
            ...
            const screenCheck = await client.query("SELECT id, is_active, screen_name FROM screens WHERE id = $1", [screen_id]);
      ...
      `;
            ...
            const availableSlots = calculateAvailableSlots(start_date, end_date, conflictResult.rows, booking_hours);
            const availability = {
...
async function createBooking(req, res) {
    try {
        ...
        const { campaign_name, advertiser_id: requestAdvertiserId, advertiser_name, screen_id, screen_name, screen_owner_id, start_date, end_date, start_time, end_time, daily_budget, total_budget, message, creative_url, creative_type, campaign_id, total_amount, booking_hours, content_url, notes, } = req.body;
        // Validation
...
// Utility functions
function calculateAvailableSlots(startDate, endDate, conflictingBookings, requestedHours) {
...
async function getCompleteBookingData(bookingId) {
    const query = `
    SELECT 
      b.*,
      s.screen_name,
    ...
    WHERE b.id = $1
  `;
    const result = await db_1.pool.query(query, [bookingId]);
    return result.rows[0] || null;
}
// ===== New Booking Request System =====
async function sendBookingRequest(req, res) {
    try {
        const { campaign_name, advertiser_id, advertiser_name, screen_id, screen_name, screen_owner_id, start_date, end_date, start_time, end_time, daily_budget, total_budget, message, creative_url, creative_type, } = req.body;
...
async function getBookedTimeSlots(req, res) {
    try {
...
async function getBookingRequestsForOwner(req, res) {
    try {
