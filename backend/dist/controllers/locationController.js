...
exports.deleteUserLocation = exports.getLocationHistory = exports.reverseGeocode = exports.getNearbyLocations = exports.updateUserLocation = exports.getUserLocation = void 0;
...
const getUserLocation = async (req, res) => {
    ...
    `;
        ...
        if (result.rows[0].is_primary) {
            await pool.query('UPDATE user_locations SET last_accessed = CURRENT_TIMESTAMP WHERE id = $1', [result.rows[0].id]);
        }
        res.json({
            success: true,
            data: {
                locations: result.rows,
                primary: result.rows.find((loc) => loc.is_primary) || result.rows[0]
            }
        });
    }
    catch (error) {
...
const updateUserLocation = async (req, res) => {
    try {
        ...
        const { latitude, longitude, accuracy, city, state, country = 'India', formattedAddress, locationSource = 'gps', locationType = 'current' } = req.body;
      ...
      `;
        ...
        `;
                ...
                if (isPrimary) {
                    await client.query('UPDATE user_locations SET is_primary = FALSE WHERE user_id = $1 AND id != $2', [userId, locationId]);
                }
            }
      ...
      `;
            await client.query(historyQuery, [
                userId, latitude, longitude, accuracy, city, state,
                country, locationSource, req.sessionID || null,
