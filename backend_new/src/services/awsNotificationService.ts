...
import { sesClient, snsClient } from "../config/aws";
...
class AWSNotificationService {
  ...
  async sendBookingNotification(data: BookingNotificationData): Promise<{
    emailSent: boolean;
    smsSent: boolean;
  }> {
    const {
  ...
  private generateCustomerConfirmationTemplate(
    ...
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px; text-align: center; margin-bottom: 30px;">
            <h1 style="color: white; margin: 0; font-size: 28px;">${statusIcon} Booking ${
      approved ? "Confirmed" : "Declined"
        ...
        <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    <td style="padding: 8px 0; font-weight: bold; color: #555;">Total Cost:</td>
                    <td style="padding: 8px 0; color: #333; font-weight: bold; font-size: 18px;">$${
                      data.totalCost
                    }</td>
                </tr>
            </table>
