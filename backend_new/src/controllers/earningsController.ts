 ...
 */
export async function getMyEarnings(req: Request & { user?: AuthUser }, res: Response) {
 ...
 */
export async function getEarningsSummary(req: Request & { user?: AuthUser }, res: Response) {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ message: 'Unauthorized' });
 ...
 */
export async function processBookingEarnings(bookingId: number) {
 ...
 */
export async function markEarningsAsPaid(earningsId: number, paymentMethod: string, reference: string) {
  try {
    const query = `
      UPDATE earnings
      SET status = 'paid',
    ...
    const result = await pool.query(query, [paymentMethod, reference, earningsId]);
