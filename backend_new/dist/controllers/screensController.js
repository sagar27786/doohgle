...
exports.getMyScreens = getMyScreens;
...
async function createScreen(req, res) {
    ...
    const { screen_name, location_in_venue, city, latitude, longitude, screen_size_inches, resolution, orientation, device_type, device_model, ads_enabled, ad_frequency, viewing_distance, typical_viewer_duration, peak_viewing_hours, day_photo_url, night_photo_url, video_url, hourly_rate, daily_rate, weekly_rate, width_px, height_px, } = req.body || {};
    ...
    try {
        const result = await db_1.pool.query(`INSERT INTO screens
...
async function getMyScreens(req, res) {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ message: "Unauthorized" });
    try {
        const result = await db_1.pool.query(`SELECT s.*
       FROM screens s
       WHERE s.user_id = $1
       ORDER BY s.created_at DESC`, [userId]);
        const screensWithAssets = await Promise.all(result.rows.map(async (row) => {
...
async function getAllScreens(req, res) {
    try {
        const { city, // Filter screens by city name
    ...
    `;
        ...
        const resultsWithPresignedUrls = await Promise.all(result.rows.map(async (row) => {
...
async function getScreenById(req, res) {
    ...
    `;
        const result = await db_1.pool.query(screenQuery, [id]);
...
async function getScreenTypes(req, res) {
    try {
        const screenTypes = [
            {
                value: "LED_billboard",
...
async function getDashboardStats(req, res) {
    try {
      ...
      WHERE user_id = $1
    `;
        const campaignStats = await db_1.pool.query(campaignStatsQuery, [userId]);
      ...
      WHERE c.user_id = $1
    `;
        const analyticsStats = await db_1.pool.query(analyticsQuery, [userId]);
      ...
      LIMIT 5
    `;
        const recentPerformance = await db_1.pool.query(recentPerformanceQuery, [
            userId,
        ]);
        res.json({
            success: true,
...
async function getRenderableUrl(urlOrKey) {
    if (!urlOrKey)
        return null;
    // If it's a URL
